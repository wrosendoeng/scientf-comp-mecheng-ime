...
def traj4DOF(time,muz, aero_coef, v2013B1120):
    ...
    #t0 = tbb.time()
    #aero_coef = pd.read_excel('PRODAS-MODIFICADO.xlsx')
    #print(f'{tbb.time() - t0} s')
    ...
    time_list = v2013B1120['Time (s)']
    dm_list = v2013B1120['dm/dt (kg/s)']
    mprop = trapz(dm_list,time_list,dx=0.02) # propellant mass with trapezoidal integration rule
    
    m0 = m1 + mprop
    
    if t <= v2013B1120['Time (s)'].iloc[-1]:
        dm = v2013B1120_function(t)
    else:
        dm = 0
