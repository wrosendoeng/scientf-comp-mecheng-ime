program interpolar
    ...
    do i = 1, nmax      ! Lendo os dados da tabela com os valores da questao
        read(unidadeleitura2,*,iostat=rc1) dadosquestao(i,:)
        print *, dadosquestao(i,:)
    !     xquestao(i) = dadosquestao(i,1)
    !     yquestao(i) = dadosquestao(i,2)
    !     print *, xquestao(i), yquestao(i)
    end do
    ...
    !     call spline3(xquestao,yquestao,nmax,x_interp,y_interp)    ! Y interpolado
    !     write(unidadeescrita2,'(f10.6,5X,f10.6)') x_interp, y_interp 
    ! end do
