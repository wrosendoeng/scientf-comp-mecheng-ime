...
PROGRAM linear_systems
    ...
    !Open Matrix A and Column Vector B from respective .txt file
    OPEN(newunit = a_unit, file='matrixA.csv', action = 'read', iostat=rc, status='old', &
    access ='sequential', form = 'formatted')
    OPEN(newunit = b_unit, file='vectorB.csv', action = 'read', iostat=rc, status='old', &
    access ='sequential', form = 'formatted')
    OPEN(newunit = x_unit, action = 'write', iostat=rc, status='replace', file='finalX.txt', &
    & access ='sequential', form = 'formatted')
    OPEN(newunit = a2_unit, action = 'write', iostat=rc, status='replace', file='finalA.txt', &
    access ='sequential', form = 'formatted')
    OPEN(newunit = b2_unit, action = 'write', iostat=rc, status='replace', file='finalB.txt', &
    access ='sequential', form = 'formatted')
